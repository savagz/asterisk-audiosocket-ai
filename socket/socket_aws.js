...
const polly = new PollyClient({
  ...
  credentials: {
    ...
    secretAccessKey: process.env.AWS_POLLY_ACCESS_KEY,
  },
});
const transcribeClient = new TranscribeStreamingClient({
  region: process.env.AWS_REGION,
  credentials: {
    accessKeyId: process.env.AWS_TRANSCRIPT_ACCESS_KEY_ID,
    secretAccessKey: process.env.AWS_TRANSCRIPT_ACCESS_KEY,
  },
});
...
function handleSocketAws(socket) {
  ...
  const _api = process.env.WEBHOOK_URL_BASE
    ? axios.create({
        baseURL: process.env.WEBHOOK_URL_BASE + process.env.WEBHOOK_URL_PORT,
      })
    : null;
  ...
  const queue = async.queue(async ({ message }) => {
    try {
      ...
      if (message === "Transferir llamada.") {
        await ami.actionRedirect(
          "CONEXION",
          socket.uuid,
          infotransfer.exten,
          infotransfer.context,
          infotransfer.priority
        );
      } else {
        ...
        for (const result of results) {
          if (result.IsPartial === false) {
            ...
            if (transcript.trim()) {
              ...
              if (_api) {
                try {
                  const response = await _api.post(process.env.WEBHOOK_URL, {
                    message: transcript,
                  });
                  _runStreaming = true;
  ...
  socket.on("error", (err) => {
    ...
    console.error(
      `[HS][AWS] - [${process.pid}] : Socket Error : `,
      err.message
    );
  });
}
